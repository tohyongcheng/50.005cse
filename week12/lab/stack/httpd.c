...
static const char *cur_dir = "."; /* web home directory -->a dot "." */
...
static void
url_decode(char *src, char *dst) //decode url for *sp1, *reqpath
{
    for (;;) {
...
static const char *
parse_req(char *reqpath)        //get the file eg. /app.py and put into reqpath buffer
{
    static char buf[8192];        /* static variables are not on the stack */

    if (fgets_trim(&buf[0], sizeof(buf), cur_f) < 0) //char *fgets(char *s, int size, FILE *stream); 
        return "Socket IO error";

    /* Parse request like "GET /foo.html HTTP/1.0" */
    char *sp1 = strchr(&buf[0], ' '); //returns a pointer to the first occurrence of ' ' in the &buf[0]
    if (!sp1)
    ...
    *sp2 = '\0';
    ...
    url_decode(sp1, reqpath);        //url_decode function for sp1 and reqpath
...
static void
process_client(int fd)   //accepted socket
{
    register int ebp __asm__ ("ebp");	//get ebp
    char reqpath[256];			//create buffer in stack
    char pn[1024];
    struct stat st;
    const char *errmsg;

    fprintf(stderr, "\nreqpath = %p\nebp     = 0x%x\nunlink  = %p\n",
            reqpath, ebp, unlink);	//memory address of reqpath and ebp

    cur_f = fdopen(fd, "w+");    //open socket associated with stream, "w+" open for r and w

    errmsg = parse_req(reqpath);	//to parse request into reqpath buffer
    if (errmsg) {
        http_err(500, "Error parsing request: %s", errmsg);
        return;
    }

    sprintf(pn, "%s/%s", cur_dir, reqpath);  //sprintf(char *str, const char *format, ...);
    //fprintf(stderr, "\npn = %s\n", pn);	//display pn array

    if (stat(pn, &st) < 0) {
        http_err(404, "File not found or not accessible: %s", pn); //print pn in size of 1024
        return;
    }
...
main(int argc, char **argv)
{
    ...
    if (listen(srvfd, 5))
        err(1, "listen");
    print_server(srvfd); //print the display message for host and post
    ...
    for (;;) {
        ...
        int cfd = accept(srvfd, (struct sockaddr *) &client_addr, &addrlen); //server accepts socket fr client
        if (cfd < 0) {
            perror("accept");
            continue;
        }
        ...
        if (pid == 0) {
            ...
            process_client(cfd);  //run process_client function for accepted client
